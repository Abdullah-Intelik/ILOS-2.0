 ...
 class CreditCardDecisionEngine {
    ...
    // SPU function - FIXED: Inverted boolean logic (TRUE = cleared, FALSE = hit)
    spu(app) {
        console.log('='.repeat(80));
        console.log('🚨 SPU MODULE CALCULATION');
        console.log('='.repeat(80));
        console.log('📥 INPUTS (TRUE = cleared, FALSE = hit):');
    ...
    scoreCity(app) {
        ...
        // Config lists (lowercase for case-insensitive matching)
        ...
        const currCity = trim(app.curr_city || "").toLowerCase();
        const officeCity = trim(app.office_city || "").toLowerCase();
    ...
    async calculateDBR(app, dbrData = null) {
        ...
        if (app) {
            ...
            const loanTenure = parseInt(app.tenure || app.requested_tenure_months || 12); // Default 12 months
            const annualRate = parseFloat(app.annual_rate || 14.6); // Default 14.6%
            ...
            // Calculate DBR using EMI instead of loan amount
            if (netIncomeValue > 0) {
                ...
                let newLoanEMI = 0;
                if (monthlyRate === 0) {
                    newLoanEMI = effectiveLoanAmount / loanTenure;
                } else {
                    newLoanEMI = effectiveLoanAmount * monthlyRate * Math.pow(1 + monthlyRate, loanTenure) /
                                (Math.pow(1 + monthlyRate, loanTenure) - 1);
                }
                
                // Calculate EMI for existing eCIB debt (if available)
                let existingDebtEMI = 0;
                if (app.ecib && app.ecib.total_exposure > 0) {
                    const existingDebt = parseFloat(app.ecib.total_exposure);
                    // Assume existing debt is amortized over 12 months at same rate
                    if (monthlyRate === 0) {
                        existingDebtEMI = existingDebt / 12;
                    } else {
                        existingDebtEMI = existingDebt * monthlyRate * Math.pow(1 + monthlyRate, 12) /
                                         (Math.pow(1 + monthlyRate, 12) - 1);
                    }
                    console.log('  • Existing eCIB Debt:', existingDebt);
                    console.log('  • Existing Debt EMI:', existingDebtEMI.toFixed(2));
                }
                
                console.log('  • New Loan EMI:', newLoanEMI.toFixed(2));
                console.log('  • Total Monthly EMI:', (newLoanEMI + existingDebtEMI).toFixed(2));
                
                // DBR calculation: (Total Monthly EMIs / Net Income) * 100
                totalObligations = newLoanEMI + existingDebtEMI;
                ...
                notes.push(`Net Income: PKR ${netIncome.toLocaleString()}`);
                notes.push(`New Loan EMI: PKR ${newLoanEMI.toLocaleString()}`);
                if (existingDebtEMI > 0) {
                    notes.push(`Existing Debt EMI: PKR ${existingDebtEMI.toLocaleString()}`);
                }
                notes.push(`Total Monthly Obligations (EMIs): PKR ${totalObligations.toLocaleString()}`);
