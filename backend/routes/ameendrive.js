...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // Normalize CNIC fields to 13 digits (strip dashes/spaces) and cap length at 13
    const toCnic = (value) => {
      const digits = normalizeCnic(value);
      if (!digits) return null;
      return String(digits).slice(0, 13);
    };
    req.body.applicant_cnic = toCnic(req.body.applicant_cnic);
    req.body.used_seller_cnic = toCnic(req.body.used_seller_cnic);
