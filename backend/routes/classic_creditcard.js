...
router.post('/', async (req, res) => {
  ...
  if (isMobileSubmission) {
    console.log('📱 Detected mobile app submission for Classic Card - will create pending_pb_completion status');
  }
    ...
    if (isMobileSubmission) {
      ...
      try {
        await client.query(`
          ...
          VALUES ($1, 'creditcard_applications', $2, $3, 'pending_pb_completion', true, $4, $5)
      ...
      setImmediate(async () => {
        try {
          const workflowResult = await processNewApplication(applicationId, {
            cnic: application.cnic || req.body.cnic,
