...
router.post('/:id/verify-document', async (req, res) => {
  const { documentId, verified, signer } = req.body;
  const { id } = req.params;

  try {
    // Get application details for LOS ID
    const appResult = await db.query('SELECT los_id FROM spu_applications WHERE id = $1', [id]);
    ...
    // Create blockchain hash for SPU document verification
    try {
      if (appResult.rows.length > 0) {
        await createFormHashAfterSubmission(
          appResult.rows[0].los_id || `SPU-${id}`,
          'SPU',
          'document_verification',
          {
            ...
            verified: verified,
            verifiedBy: signer || 'spu_staff',
            verificationTimestamp: new Date().toISOString()
          },
          signer || 'spu_staff'
        );
        console.log(`✅ Blockchain hash created for SPU document verification of app ${id}`);
      }
    } catch (hashError) {
      console.error('⚠️ Failed to create blockchain hash for document verification:', hashError.message);
