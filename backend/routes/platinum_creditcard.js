...
router.post('/', async (req, res) => {
  ...
  if (isMobileSubmission) {
    console.log('📱 Detected mobile app submission for Platinum Card - will create pending_pb_completion status');
  }
    ...
    if (isMobileSubmission) {
      ...
      try {
        await client.query(`
          ...
          VALUES ($1, 'platinum_card_applications', $2, $3, 'pending_pb_completion', true, $4, $5)
        ...
        `, [
          applicationId, 
          application.nic || req.body.nic || null, 
      ...
      setImmediate(async () => {
        try {
          const workflowResult = await processNewApplication(applicationId, {
            cnic: application.nic || req.body.nic,
