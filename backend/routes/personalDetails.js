...
// POST route to insert personal details with blockchain hash
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // Create blockchain hash for the form submission
    ...
    // Return response with hash information
    res.status(201).json({
      success: true,
      data: result.rows[0],
      blockchain: {
    ...
    // Even if database insertion fails, create blockchain hash
    const department = req.headers['x-department'] || 'PB';
    const signer = req.headers['x-signer'] || 'system_user';
    ...
    try {
      ...
      console.log('✅ Blockchain hash created despite database failure');
    } catch (hashErr) {
      console.error('❌ Blockchain hash creation also failed:', hashErr.message);
      hashResult = { success: false, error: hashErr.message };
    }

    // Return response with blockchain hash even if database failed
    res.status(207).json({
      success: true,
      database: {
        inserted: false,
      ...
      message: 'Blockchain hash created (database insertion failed)'
    });
  }
});
